...
Use this when the research output involves `.docx`, `.pdf`, `.pptx`, `.xlsx`, frontend design, or shareable web artifacts. Coresearch owns research content and claim integrity; companion skills/tools own their format mechanics when installed.
...
| Generic PDF extraction/edit/merge/OCR | Verify content and citation/source faithfulness | `pdf`; open-access literature crawl runs inside `research-survey` |
| Talk plan or slide narrative | `research-slides` creates claim-faithful plan | Claude `pptx` companion for finished PowerPoint mechanics; `academic-ppt` only for paper-faithful/equation-heavy builds |
...
- **PPTX:** route to the Claude `pptx` companion for PowerPoint creation/editing and visual QA. Use `academic-ppt` for LaTeX/PDF extraction, equations, and source-faithful defense decks. Render slides to images or thumbnails, inspect for placeholder text, overflow, broken fonts, unreadable figures, and claim drift.
...
For research decks, use an argument-first academic structure: audience/background → gap/tension → thesis → method logic → evidence sequence → limits → takeaway. Visual polish must serve this spine. Route file mechanics through the Claude `pptx` companion (or `academic-ppt` for source-faithful builds).
