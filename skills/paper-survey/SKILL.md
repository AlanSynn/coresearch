---
name: paper-survey
description: Verified literature survey and related-work synthesis for research topics. Use when the user asks for related work, papers on a topic, citation categories, a literature map, novelty risks, survey writing, or paper recommendations. Requires real-paper verification, pdf-crawl-compatible evidence tables when useful, and explicit metadata/PDF confidence; never fabricates citations, venues, or paper claims.
---
...
When exactness matters, when the user says not to assume, or when the task asks for a paper list/roadmap, align with `pdf-crawl`:
...
2. If a file path is available and downloading is requested or useful for verification, run a dry run first:
   ...
   If this repository path is not current, resolve the crawler relative to the `pdf-crawl` skill folder.

3. For real downloads, follow `pdf-crawl` guardrails exactly: open-access sources by default, no paywall bypassing, fuzzy DOI matches are candidates only, and lists over 30 papers require reviewed dry-run output plus `--yes-large`.
...
5. If the user only requested an in-chat survey and did not request file writes/downloads, do not create files solely for process; instead include a `PDF Crawl Roadmap` table that can be saved and passed to `pdf-crawl`.
...
3. **Create the evidence ledger.** Record candidates in a `pdf-crawl`-compatible roadmap table or equivalent in-chat table before synthesis.
...
5. **Crawl or inspect paper text when needed.** Use `pdf-crawl` for batch open-access PDFs when a roadmap file is available; otherwise inspect official PDFs/full text directly. Do not upgrade beyond `METADATA VERIFIED` without paper text.
...
9. **Output sources.** Include links used and any `pdf-crawl` command/output summary when run.
...
## PDF Crawl Roadmap
Use this when PDFs should be fetched with `pdf-crawl`; omit if no crawl/download workflow is needed.
...
## Verification Notes
- pdf-crawl command/output summary if run:
...
- fuzzy `pdf-crawl` or DOI matches are not treated as verified papers;
- papers that `pdf-crawl` reports as not found are not cited as established evidence unless independently verified elsewhere;
